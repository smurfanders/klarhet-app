...
const t = {
  en: {
    ...
    heroSub: 'Your honest feedback helps this candidate understand where to focus. There are no wrong answers.',
    q: [
      {
        num: 'Question 1 of 7',
        text: 'How well did the candidate\'s experience match what you were looking for?',
        opts: [['strong','✦ Strong match'],['partial','◈ Partial match'],['notfit','○ Not quite the right fit']],
      ...
      },
      {
        ...
        opts: [['excellent','✦ Excellent'],['good','◈ Good'],['develop','○ Has room to grow']],
        condTrigger: 'develop',
        condLabel: 'Where specifically? Pick all that apply.',
        checkboxes: [['clarity','Clarity of answers'],['structure','Structuring thoughts'],
                     ['listening','Active listening'],['confidence','Confidence'],['conciseness','Conciseness']],
      },
      {
        num: 'Question 3 of 7',
      ...
      },
      {
        ...
        opts: [['yes','✦ Yes, definitely'],['maybe','◈ Possibly, for the right role'],['unlikely','○ Unlikely']],
      ...
      },
      {
        ...
        stars: true,
        starLabels: ['Needs significant work','Below expectations','Met expectations','Strong candidate','Outstanding'],
      },
      {
        num: 'Question 6 of 7',
        ...
        freeText: true,
        placeholder: 'e.g. Getting hands-on experience with enterprise CRMs would go a long way…',
      },
      {
    ...
    next: 'Continue', back: 'Back', submit: 'Send feedback ✓',
    successTitle: 'Thank you so much.',
    successBody: 'Your feedback will make a real difference. Honest, specific input like this is genuinely rare — and it helps people grow in ways that a rejection alone never could.',
  ...
  sv: {
    ...
    heroSub: 'Din ärliga feedback hjälper kandidaten att förstå vad de ska fokusera på. Det finns inga fel svar.',
    q: [
      {
        num: 'Fråga 1 av 7',
        text: 'Hur väl matchade kandidatens erfarenhet det ni sökte?',
        opts: [['strong','✦ Stämde väl'],['partial','◈ Delvis matchande'],['notfit','○ Passade inte riktigt']],
      ...
      },
      {
        ...
        opts: [['excellent','✦ Utmärkt'],['good','◈ Bra'],['develop','○ Har utrymme att växa']],
        condTrigger: 'develop',
        condLabel: 'Var specifikt? Välj allt som stämmer.',
        checkboxes: [['clarity','Tydlighet i svar'],['structure','Strukturerat tänkande'],
                     ['listening','Aktivt lyssnande'],['confidence','Självförtroende'],['conciseness','Koncishet']],
      },
      {
        num: 'Fråga 3 av 7',
      ...
      },
      {
        ...
        opts: [['yes','✦ Ja, absolut'],['maybe','◈ Möjligen, för rätt roll'],['unlikely','○ Troligen inte']],
        condTrigger: 'unlikely',
      ...
      },
      {
        ...
        stars: true,
        starLabels: ['Behöver mycket arbete','Under förväntningarna','Uppfyllde förväntningarna','Stark kandidat','Enastående'],
      },
      {
        num: 'Fråga 6 av 7',
        ...
        freeText: true,
        placeholder: 't.ex. Att skaffa praktisk erfarenhet av enterprise-CRM-system skulle göra stor skillnad…',
      },
      {
    ...
    next: 'Fortsätt', back: 'Tillbaka', submit: 'Skicka feedback ✓',
    successTitle: 'Tack så hjärtligt.',
    successBody: 'Din feedback gör verklig skillnad. Ärlig, specifik input som denna är genuint sällsynt — och den hjälper människor att växa på sätt som ett avslag ensamt aldrig kan.',
...
export default function FeedbackPage({ params }: { params: { token: string } }) {
  ...
  const [status, setStatus] = useState<'loading'|'ready'|'notfound'|'done'|'submitted'>('loading')
  ...
  async function handleSubmit() {
    ...
    const res = await fetch(`/api/form/${params.token}/submit`, {
      method: 'POST',
  ...
  if (status === 'loading') {
    return <div style={f.bg}><div style={f.loading}>Loading…</div></div>
  }
    ...
    return (
      <div style={f.bg}>
        <div style={f.successWrap}>
          ...
          <div style={f.successPill}>{company} · {role}</div>
        </div>
      </div>
    )
  }
        ...
        <div style={f.card}>
              ...
              {q.opts.map(([val, label]: string[]) => {
                ...
                const showCond =
                  selected && (
                    ...
                    (!q.condTrigger && !q.condTriggers && val === 'notfit' && q.cond)
                  )
                return (
                  <div key={val}>
                    <div
                    ...
                    >
                      <div style={{ ...f.radio, ...(selected ? f.radioSelected : {}) }}>
                    ...
                    {selected && q.cond && !q.checkboxes && (
                      <div style={f.conditional}>
                        ...
                        <textarea
                          ...
                          onChange={e => setTextFields(p => ({ ...p, [`q${step}_detail`]: e.target.value }))}
                        />
                      </div>
                    )}
                    ...
                    {selected && q.checkboxes && showCond && (
                      <div style={f.conditional}>
                        ...
                        {q.checkboxes.map(([cbVal, cbLabel]: string[]) => (
                          <div
                            key={cbVal}
                            style={{ ...f.checkbox, ...(checkboxes.includes(cbVal) ? f.checkboxChecked : {}) }}
                            onClick={() => toggleCheckbox(cbVal)}
                          >
                            <div style={{ ...f.checkboxBox, ...(checkboxes.includes(cbVal) ? f.checkboxBoxChecked : {}) }}>
            ...
            <div>
              <div style={f.stars}>
                {[1,2,3,4,5].map(n => (
                  <span
                    key={n}
                    style={{ ...f.star, color: n <= (starHover || starRating) ? '#e8b86d' : '#252936' }}
          ...
          {q.freeText && (
            <textarea
              ...
              onChange={e => setTextFields(p => ({ ...p, [`q${step}_text`]: e.target.value }))}
            />
          )}
          ...
          {q.dualText && (
            <div>
              <textarea
                ...
                onChange={e => setTextFields(p => ({ ...p, [`q${step}_text`]: e.target.value }))}
              />
              <div style={{ fontSize: '13px', color: '#5a6080', marginBottom: '8px' }}>{q.label2}</div>
              <textarea
                style={f.textarea}
                rows={3}
                placeholder={q.placeholder2}
                value={textFields[`q${step}_text2`] ?? ''}
                onChange={e => setTextFields(p => ({ ...p, [`q${step}_text2`]: e.target.value }))}
              />
            </div>
          )}
        </div>
...
const f: Record<string, React.CSSProperties> = {
  bg: { minHeight: '100vh', background: '#f5f0e8', fontFamily: "'DM Sans', system-ui, sans-serif" },
  loading: { padding: '80px', textAlign: 'center', color: '#8a8070', fontSize: '14px' },
  msgCard: { maxWidth: '420px', margin: '120px auto', textAlign: 'center', padding: '24px' },
  ...
  eyebrow: { fontSize: '11px', letterSpacing: '3px', textTransform: 'uppercase',
             color: '#e8a96a', marginBottom: '12px', fontWeight: 500 },
  heroTitle: { fontFamily: 'Georgia, serif', fontSize: '32px', color: '#f5f0e8',
               lineHeight: 1.2, marginBottom: '14px', fontWeight: 400 },
  heroSub: { color: 'rgba(245,240,232,0.6)', fontSize: '15px', lineHeight: 1.6,
             fontWeight: 300, marginBottom: 0 },
  contextPill: { display: 'inline-flex', alignItems: 'center', gap: '8px',
                 background: 'rgba(255,255,255,0.07)', border: '1px solid rgba(255,255,255,0.1)',
                 borderRadius: '40px', padding: '8px 18px', marginTop: '22px',
                 fontSize: '13px', color: 'rgba(245,240,232,0.7)' },
  dot: { width: '6px', height: '6px', borderRadius: '50%', background: '#e8a96a', flexShrink: 0 },
  progressWrap: { maxWidth: '600px', margin: '0 auto', padding: '18px 24px 12px',
                  borderBottom: '1px solid #d4c8b8', background: '#f5f0e8',
                  position: 'sticky', top: 0, zIndex: 10 },
  progressMeta: { marginBottom: '8px' },
  progressLabel: { fontSize: '11px', color: '#8a8070', letterSpacing: '1px',
                   textTransform: 'uppercase', fontWeight: 500 },
  progressTrack: { height: '3px', background: '#d4c8b8', borderRadius: '4px', overflow: 'hidden' },
  progressFill: { height: '100%', background: 'linear-gradient(90deg, #c17d3c, #e8a96a)',
                  borderRadius: '4px', transition: 'width 0.4s ease' },
  formWrap: { maxWidth: '600px', margin: '0 auto', padding: '28px 24px 60px' },
  card: { background: '#ffffff', borderRadius: '14px', padding: '28px',
          border: '1px solid #d4c8b8', boxShadow: '0 2px 16px rgba(26,26,46,0.06)',
          marginBottom: '20px' },
  qText: { fontFamily: 'Georgia, serif', fontSize: '20px', color: '#1a1a2e',
           lineHeight: 1.4, marginBottom: '6px' },
  qSub: { fontSize: '13px', color: '#8a8070', marginBottom: '20px', lineHeight: 1.5 },
  opts: { display: 'flex', flexDirection: 'column', gap: '8px', marginTop: '16px' },
  opt: { display: 'flex', alignItems: 'center', gap: '12px', padding: '13px 16px',
         border: '1.5px solid #d4c8b8', borderRadius: '10px', cursor: 'pointer',
         background: '#f5f0e8', transition: 'all 0.15s' },
  optSelected: { borderColor: '#c17d3c', background: 'rgba(193,125,60,0.07)' },
  radio: { width: '20px', height: '20px', borderRadius: '50%', border: '2px solid #d4c8b8',
           flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' },
  radioSelected: { borderColor: '#c17d3c', background: '#c17d3c' },
  radioDot: { width: '8px', height: '8px', borderRadius: '50%', background: 'white' },
  conditional: { marginTop: '10px', marginLeft: '8px', background: 'rgba(193,125,60,0.05)',
                 border: '1px dashed #c17d3c', borderRadius: '10px', padding: '14px' },
  condLabel: { fontSize: '12px', color: '#c17d3c', fontWeight: 500, marginBottom: '10px' },
  checkbox: { display: 'flex', alignItems: 'center', gap: '10px', padding: '9px 12px',
              borderRadius: '8px', cursor: 'pointer', border: '1px solid #d4c8b8',
              marginBottom: '6px', background: '#f5f0e8', fontSize: '13px' },
  checkboxChecked: { borderColor: '#c17d3c', background: 'rgba(193,125,60,0.08)' },
  checkboxBox: { width: '18px', height: '18px', borderRadius: '4px', border: '2px solid #d4c8b8',
                 flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center',
                 fontSize: '11px' },
  checkboxBoxChecked: { background: '#c17d3c', borderColor: '#c17d3c', color: 'white' },
  stars: { display: 'flex', gap: '8px', marginTop: '16px' },
  star: { fontSize: '40px', cursor: 'pointer', transition: 'color 0.1s, transform 0.1s',
          userSelect: 'none', lineHeight: 1 },
  starLabel: { fontSize: '13px', color: '#8a8070', marginTop: '10px', fontStyle: 'italic' },
  textarea: { width: '100%', border: '1.5px solid #d4c8b8', borderRadius: '8px',
              padding: '11px 13px', fontFamily: 'inherit', fontSize: '14px',
              color: '#1a1a2e', background: '#f5f0e8', resize: 'vertical',
              outline: 'none', lineHeight: 1.5, boxSizing: 'border-box', marginTop: '8px' },
  nav: { display: 'flex', justifyContent: 'space-between', alignItems: 'center' },
  btnBack: { background: 'none', border: 'none', color: '#8a8070', fontFamily: 'inherit',
             fontSize: '14px', cursor: 'pointer', padding: '10px 0' },
  btnNext: { background: '#1a1a2e', color: '#f5f0e8', border: 'none', borderRadius: '10px',
             padding: '13px 26px', fontFamily: 'inherit', fontSize: '15px',
             fontWeight: 500, cursor: 'pointer' },
  btnDisabled: { opacity: 0.35, cursor: 'not-allowed' },
  successWrap: { maxWidth: '480px', margin: '0 auto', padding: '80px 24px', textAlign: 'center' },
  successTitle: { fontFamily: 'Georgia, serif', fontSize: '30px', color: '#1a1a2e',
                  marginBottom: '12px', fontWeight: 400 },
  successBody: { color: '#8a8070', fontSize: '16px', lineHeight: 1.6, fontWeight: 300, marginBottom: '28px' },
  successPill: { display: 'inline-block', background: 'rgba(193,125,60,0.1)',
                 border: '1px solid rgba(193,125,60,0.3)', borderRadius: '40px',
                 padding: '8px 20px', fontSize: '13px', color: '#c17d3c' },
}
