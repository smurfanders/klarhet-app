...
export async function GET(
  ...
  try {
    ...
    ).get(token) as { id: string; company: string; role: string; language: string } | undefined
    ...
    return NextResponse.json({
      data: { company: application.company, role: application.role, language: application.language },
      error: null,
    })
  } catch (err) {
